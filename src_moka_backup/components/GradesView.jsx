...
const GradesView = ({ studentData, searchQuery = '' }) => {
  ...
  const realGrades = gradesData || {};

  const [isDownloading, setIsDownloading] = useState(false);

  const [onlineExams, setOnlineExams] = useState(() => {
    try { 
      const t1 = teacherTests || [];
      const t2 = globalExams || [];
      return [...t1, ...t2];
    } catch { return []; }
  });
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 no-print">
        {[
          ...
          <div key={idx} className="glass-card rounded-3xl p-8 shadow-sm flex items-center justify-between group">
            <div>
      ...
      <div className="glass-card rounded-3xl shadow-sm overflow-hidden border border-white/20">
        <div className="p-8 border-b border-gray-100 flex items-center justify-between no-print">
            ...
            return (
              <div key={subject?.id} className="group">
                <div className="flex items-end justify-between mb-4">
                  <div>
                    <h3 className="text-xl font-bold text-gray-800 mb-1 group-hover:text-primary transition-colors">{subject?.name || 'مادة غير معروفة'}</h3>
                  ...
                  <div className="text-left">
                    <span className={`inline-block px-4 py-1.5 rounded-xl ${style.bg} ${style.label} text-sm font-bold shadow-sm border border-black/5 mb-1`}>
                      {subject?.grade || '-'} | {subject?.percentage || 0}%
                    </span>
                    <p className={`text-[10px] font-bold uppercase tracking-wider ${style.label} text-center`}>{style.text}</p>
                  </div>
                </div>
      ...
      <div className="mt-12 no-print">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {(() => {
            let achievements = [];
            try {
              achievements = (achievementsData || []).filter(a => a.studentId === studentData?.personal?.id);
            } catch { achievements = []; }
            ...
            return achievements.map((achievement) => (
              <div key={achievement?.id || Math.random()} className="glass-card rounded-3xl p-6 shadow-sm hover:-translate-y-2 transition-all duration-500 text-center group">
      ...
      <div id="certificate-print-area" className="hidden print:block fixed inset-0 bg-white z-[9999] p-10 font-main w-full min-h-screen text-black" dir="rtl">
        {/* إطار فخم */}
        <div id="certificate-inner-border" className="border-4 border-double border-gray-300 p-8 min-h-[calc(100vh-5rem)] flex flex-col relative bg-white">
          
          {/* علامة مائية */}
          <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none">
             <img src={logoImage} alt="Watermark" className="w-[500px] grayscale" />
          </div>
          ...
          {/* بيانات الطالب */}
          <div className="grid grid-cols-2 gap-y-4 gap-x-8 mb-10 relative z-10 bg-gray-50 p-6 rounded-xl border border-gray-100">
