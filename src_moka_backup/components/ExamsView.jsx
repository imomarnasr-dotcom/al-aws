...
const cleanAndFetchExams = (teacherTests, globalExams) => {
  try {
    const fromTeacher = teacherTests || [];
    const fromOld = globalExams || [];
    ...
    // لم نعد بحاجة לחفظ localStorage هنا لأن useCloudStorage هو المصدر الأساسي
...
const ExamsView = ({ studentData, setStudentData, searchQuery = '' }) => {
  ...
  const [globalExams, setGlobalExams] = useCloudStorage('exams', 'all_data', []);
  const [gradesData, setGradesData] = useCloudStorage('grades', 'all_data', {});
  const [exams, setExams] = useState(() => cleanAndFetchExams(teacherTests, globalExams));
  const [activeExam, setActiveExam] = useState(null);

  useEffect(() => {
    setExams(cleanAndFetchExams(teacherTests, globalExams));
  }, [teacherTests, globalExams]);
  ...
  useEffect(() => {
    const handleStorage = () => { setExams(cleanAndFetchExams(teacherTests, globalExams)); };
  ...
  const submitExam = (auto = false) => {
    ...
    const currentExams = cleanAndFetchExams(teacherTests, globalExams);
    ...
    // 🔥 الإصلاح: setStudentData في App.jsx هو handleSetStudentData الذي يزيد syncTrigger فقط
  ...
  const startExam = (exam) => {
    ...
    try {
      const freshExams = cleanAndFetchExams(teacherTests, globalExams);
  ...
  if (activeExam) {
    ...
    return (
      <div className="fixed inset-0 z-[1000] bg-gray-50 flex flex-col font-main" dir="rtl">
        {toast && (
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[2000] bg-red-600 text-white px-8 py-4 rounded-full shadow-2xl font-black text-sm animate-bounce">
            {toast}
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col items-center justify-center w-full">
          <div className="w-full max-w-3xl">
            ...
            <div className="bg-white p-8 md:p-12 rounded-[40px] shadow-lg border border-gray-100 animate-fade-in-up relative overflow-hidden min-h-[400px] flex flex-col">
    ...
    <div className="flex-1 space-y-8 pb-10 relative">
      ...
      {examToStart && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-gray-900/70 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl animate-fade-in-up text-center border border-gray-100">
      ...
      {showResult && (
        <div className="space-y-6 animate-fade-in-up max-w-3xl mx-auto">
          {/* بطاقة الدرجة */}
          <div className="bg-white rounded-[40px] p-10 shadow-2xl shadow-primary/5 border border-gray-100 text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 w-full h-2 bg-gradient-to-l from-primary to-emerald-400"></div>
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-2">تم تسليم الاختبار بنجاح!</h2>
            <p className="text-6xl font-black text-primary mb-2">{showResult.score}/{showResult.total}</p>
            <p className="text-lg font-bold text-gray-500 mb-2">
              {showResult.total > 0 ? Math.round((showResult.score / showResult.total) * 100) : 0}%
            </p>
            <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-6">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${
                  showResult.total > 0 && (showResult.score / showResult.total) >= 0.9 ? 'bg-emerald-500' :
                  (showResult.score / showResult.total) >= 0.6 ? 'bg-amber-400' : 'bg-red-500'
                }`}
                style={{ width: `${showResult.total > 0 ? (showResult.score / showResult.total) * 100 : 0}%` }}
              />
            </div>
            <p className="text-gray-500 font-medium bg-gray-50 p-4 rounded-2xl border border-gray-100 text-sm">
              {showResult.message}
            </p>
          </div>
      ...
      {!showResult && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {myExams.length > 0 ? myExams.map(exam => {
            ...
            return (
              <div key={exam.id} className="glass-card rounded-[32px] p-6 shadow-sm border border-gray-100 flex flex-col group hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                {isDone && (
                  <div className="absolute -left-10 top-6 -rotate-45 bg-emerald-500 text-white font-bold text-[10px] py-1 px-10 shadow-sm z-10 text-center">
                    تم التسليم
                  </div>
                )}
                {isMissed && (
                  <div className="absolute -left-10 top-6 -rotate-45 bg-red-500 text-white font-bold text-[10px] py-1 px-10 shadow-sm z-10 text-center">
                    غير حاضر
                  </div>
                )}
