...
const StudentIdGenerator = () => {
  const [students] = useCloudStorage('whitelist', 'all_data', []);
  ...
  const generateCardHTML = (student, type, mode) => {
    ...
    const qrSvg = ReactDOMServer.renderToStaticMarkup(
      <QRCodeSVG value={student.id.toString()} size={type === 'id' ? 240 : 350} level="H" includeMargin={false} fgColor={theme.dark} />
    );
        ...
        <div style={{ display: 'flex', padding: '12px', gap: '12px', flex: 1, alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              <div style={{ background: theme.light, padding: '2px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ background: 'white', padding: '6px', borderRadius: '12px', border: `2px solid ${theme.mid}`, boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <QRCodeSVG value={student.id.toString()} size={70} level="H" includeMargin={false} fgColor={theme.dark} />
