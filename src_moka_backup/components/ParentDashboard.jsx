...
const ParentDashboard = ({ studentData, onLogout }) => {
  ...
  const [cloudBehaviorLogs] = useCloudStorage('behavior_logs', 'all_data', {});
  const [cloudGrades] = useCloudStorage('grades', 'all_data', {});
  const [teacherTests] = useCloudStorage('tests', 'all_data', []);
  const [examsData] = useCloudStorage('exams', 'all_data', []);
  const [achievementsData] = useCloudStorage('achievements', 'all_data', {});
  const [walletsData] = useCloudStorage('wallets', 'all_data', {});
  const [walletTxnsData] = useCloudStorage('wallet_transactions', 'all_data', []);
  const [complaintsData, setComplaintsData] = useCloudStorage('complaints', 'all_data', []);
  ...
  }, [studentData, parentNotifs, cloudBehaviorLogs, cloudGrades, teacherTests, examsData, achievementsData, walletsData, walletTxnsData, complaintsData]);
      ...
      <header className="bg-white/95 backdrop-blur-md border-b border-slate-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <img src={logoImage} alt="logo" className="w-9 h-9 rounded-xl object-contain border border-slate-100 hidden md:block" onError={e => e.target.style.display='none'} />
            <div>
