...
import { getAllDocs, getSingleDoc } from '../services/dbService';
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (activePortal === 'admin') {
        ...
        const staffDoc = await getSingleDoc('staff', 'all_data');
        ...
        if (adminStaff) {
          setIsLoggingIn(false);
          return onNavigate('admin', adminStaff);
        }
      ...
      if (activePortal === 'student') {
        const whitelistDoc = await getSingleDoc('whitelist', 'all_data');
        const whitelist = whitelistDoc?.data || [];
        const student = whitelist.find(s => 
          s.id.toLowerCase().replace(/-/g, '') === loginData.id.toLowerCase().replace(/-/g, '') &&
          s.name.trim() === loginData.name.trim() &&
          s.password === loginData.password &&
        ...
        setIsLoggingIn(false);
        if (student) {
          return onNavigate('home', student);
        } else {
      ...
      if (activePortal === 'parent') {
        ...
        const student = whitelist.find(s => 
          s.id.toLowerCase().replace(/-/g, '') === loginData.id.toLowerCase().replace(/-/g, '') &&
          s.parentName?.trim() === loginData.name.trim() &&
          s.parentPassword === loginData.password
        );

        setIsLoggingIn(false);
        if (student) {
          return onNavigate('parent-dashboard', student);
        } else {
      ...
      <div className="glass-card rounded-[40px] p-8 md:p-12 max-w-2xl w-full border border-white/60 relative z-10 hover-glow group">
        ...
        <div className="text-center mb-8 relative">
          ...
          <div className="w-24 h-24 mx-auto flex items-center justify-center shadow-2xl shadow-primary/20 mb-6 relative z-10 group-hover:scale-110 transition-all duration-500 bg-white rounded-3xl border border-gray-100 p-2">
            <img src={logoImage} alt="Logo" className="w-full h-full object-contain rounded-2xl" />
          </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5 animate-fade-in">
          ...
          <button
            ...
            className={`w-full bg-gradient-to-r from-gray-900 to-gray-800 text-white rounded-2xl py-4 font-bold text-base shadow-xl shadow-gray-900/20 transition-all flex items-center justify-center gap-2 group mt-6 border border-gray-700 ${isLoggingIn ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-2xl hover:shadow-gray-900/30 hover:-translate-y-1'}`}
          >
            {isLoggingIn ? 'جاري التحقق...' : 'تسجيل الدخول'}
            {!isLoggingIn && <ArrowLeft size={20} className="group-hover:-translate-x-2 transition-transform" />}
          </button>
        </form>
