...
const CafeteriaView = ({ studentData, cart, setCart, searchQuery = '', studentBalance }) => {
  ...
  const [liveMenu] = useCloudStorage('cafeteria_menu', 'all_data', []);
  const [ordersData] = useCloudStorage('cafeteria_orders', 'all_data', []);
  
  const transactions = useMemo(() => {
    const myId = studentData?.personal?.id;
    if (myId) {
      return ordersData.filter(o => o.studentId === myId).slice(0, 50);
    }
    return [];
  }, [ordersData, studentData?.personal?.id]);
  ...
  const addToCart = (item) => {
    ...
    if (existing) {
      if (existing.cartQuantity >= item.quantity) {
        window.dispatchEvent(new CustomEvent('moo-toast', { detail: { message: 'عذراً، لا يمكنك إضافة المزيد من هذا الصنف.', type: 'error' } }));
        return;
      }
      setCart(cart.map(c => c.id === item.id ? { ...c, cartQuantity: c.cartQuantity + 1 } : c));
    } else {
      if (item.quantity <= 0) {
        window.dispatchEvent(new CustomEvent('moo-toast', { detail: { message: 'الكمية غير متوفرة.', type: 'error' } }));
        return;
      }
