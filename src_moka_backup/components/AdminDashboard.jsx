...
import {
  ...
  Download, Upload, Database, Search, Wallet
} from 'lucide-react';
...
const AdminDashboard = ({ onLogout, account }) => {
  ...
  const [phases, setPhases] = useCloudStorage('phases', 'all_data', defaultPhases);
  ...
  const [announcements, setAnnouncements] = useCloudStorage('announcements', 'all_data', []);
  ...
  // Cloud sync handled by useCloudStorage
    ...
    if (oldId !== editingStudent.id) {
      try {
        ...
        if (Object.prototype.hasOwnProperty.call(wallets, oldId)) {
          currentWallets[editingStudent.id] = currentWallets[oldId];
          delete currentWallets[oldId];
  ...
  const executeWalletTransaction = () => {
    ...
    const currentWallets = { ...wallets };
    const currentBalance = currentWallets[student?.id] || 0;
    ...
    currentWallets[student?.id] = newBalance;
        ...
        {!showScheduleOverlay && (
          <>
            <motion.header
              ...
              className="header-modern bg-white/80 backdrop-blur-xl border-b border-white/20 px-8 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm"
            >
              <div className="flex items-center gap-4">
                <img src={logoImage} alt="Logo" className="w-12 h-12 rounded-xl object-contain shadow-sm border border-gray-100 bg-white" />
                <div>
                  <h1 className="text-xl font-bold text-gray-900">مدارس الأوس الأهلية</h1>
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest mt-0.5">لوحة المسؤول الشاملة</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="hidden md:flex flex-col items-end text-sm font-bold text-gray-700">
                  <div className="flex items-center gap-2">
                    <span className="text-primary">{currentDateTime.toLocaleDateString('ar-SA-u-ca-islamic', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                ...
                <button onClick={onLogout} className="btn-secondary flex items-center gap-2 px-5 py-2.5 btn-oversized">
                  خروج
                  <LogOut size={18} />
                </button>
              </div>
            ...
            <main className="flex-1 max-w-7xl mx-auto w-full p-8 flex gap-8">
              <motion.div
                ...
                className="sidebar-modern w-64 shrink-0 space-y-3"
              >
                {[
                  ...
                  <button
                    ...
                    className={`sidebar-item w-full px-5 py-4 rounded-2xl font-bold text-sm transition-all ${
                      activeTab === tab?.id
                        ? 'bg-gradient-to-l from-gray-900 to-gray-800 text-white shadow-xl shadow-gray-900/20 translate-x-[-4px]'
                  ...
                  >
                    ...
                    {activeTab === 'stats' && (
                      <div className="space-y-8">
                        <div className="border-b border-gray-200/50 pb-6 flex items-center justify-between flex-wrap gap-4">
                          <div>
                            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><BarChart2 className="text-emerald-500" /> إحصائيات النظام</h2>
                            <p className="text-sm text-gray-400 mt-1">نظرة شاملة على بيانات المدرسة + النسخ الاحتياطي</p>
                          </div>
                          {/* 🔥 أزرار النسخ الاحتياطي وتصدير التقرير */}
                          <div className="flex flex-wrap items-center gap-3">
                            <button onClick={handleExportPDF} disabled={isExportingPDF} className={`flex items-center gap-2 ${isExportingPDF ? 'bg-emerald-400' : 'bg-emerald-600 hover:bg-emerald-700'} text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-md`}>
                            ...
                            <button onClick={handleExportBackup} className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-800 transition-colors shadow-md">
                              <Download size={18} /> تصدير نسخة
                            </button>
                            <label className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm cursor-pointer">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                          {stats.map((s, i) => (
                            <div key={i} className="glass-card bg-white/70 p-6 rounded-3xl border border-white/50 shadow-sm flex items-center gap-4">
                                ...
                                return (
                                  <div className="space-y-6">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                      ...
                                      <div className="glass-card bg-white/70 rounded-3xl p-6 border border-white/50 shadow-xl flex flex-col items-center">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                      <div className="glass-card bg-emerald-50/50 rounded-3xl p-6 border border-emerald-100 shadow-sm">
                                      ...
                                      <div className="glass-card bg-rose-50/50 rounded-3xl p-6 border border-rose-100 shadow-sm">
                                      ...
                                      <div className="glass-card bg-amber-50/50 rounded-3xl p-6 border border-amber-100 shadow-sm">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                          <div className="lg:col-span-1 space-y-4">
                            <div className="glass-card bg-white/70 p-6 rounded-3xl border border-white/50 shadow-xl">
                              ...
                              <form onSubmit={handleSaveAnnouncement} className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    ...
                                    <input type="datetime-local" value={newAnnouncement.startDate} onChange={e => setNewAnnouncement({...newAnnouncement, startDate: e.target.value})} className="w-full border border-gray-200 rounded-xl p-2 text-xs outline-none" />
                                  </div>
                                  <div>
                                    <label className="text-xs text-gray-500 font-bold mb-1 block">تاريخ الانتهاء (اختياري)</label>
                                    <input type="datetime-local" value={newAnnouncement.endDate} onChange={e => setNewAnnouncement({...newAnnouncement, endDate: e.target.value})} className="w-full border border-gray-200 rounded-xl p-2 text-xs outline-none" />
                                  </div>
                                </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 space-y-6 shadow-xl">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                              <div>
                                ...
                                <input type="time" value={settings.firstLessonStart} onChange={(e) => setSettings({ ...settings, firstLessonStart: e.target.value })} className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all shadow-sm" />
                              </div>
                              <div>
                                <label className="text-xs text-gray-500 mb-1 block font-bold">مدة الحصة (د)</label>
                                <input type="number" min="20" value={settings.lessonDuration} onChange={(e) => setSettings({ ...settings, lessonDuration: Number(e.target.value) })} className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all shadow-sm" />
                              </div>
                              <div>
                                <label className="text-xs text-gray-500 mb-1 block font-bold">عدد الحصص</label>
                                <input type="number" min="3" value={settings.lessonsPerDay} onChange={(e) => setSettings({ ...settings, lessonsPerDay: Number(e.target.value) })} className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all shadow-sm" />
                              </div>
                              <div className="bg-orange-50/80 p-2.5 rounded-xl border border-orange-100 shadow-sm col-span-2 md:col-span-1">
                              ...
                              <div className="bg-orange-50/80 p-2.5 rounded-xl border border-orange-100 shadow-sm col-span-2 md:col-span-1">
                                ...
                                <input type="number" min="5" value={settings.breakDuration || 30} onChange={(e) => setSettings({ ...settings, breakDuration: Number(e.target.value) })} className="w-full bg-white border border-orange-200 rounded-lg px-2 py-1.5 text-sm text-orange-900 focus:ring-2 focus:ring-orange-500/20 outline-none transition-all" />
                              </div>
                            </div>
                            <button onClick={() => {
                              setPasswordModalConfig({
                          ...
                          <div className="border-t border-gray-200/50 pt-8 mt-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                              {phases.map(phase => (
                                <div key={phase.id} className="bg-white/60 p-6 rounded-3xl border border-white/50 shadow-sm hover:shadow-md transition-shadow">
                           ...
                           <div className="border-t border-gray-200/50 pt-6">
                             <button
                               ...
                               className={`btn-oversized w-full px-6 py-4 rounded-2xl font-bold text-lg transition-all ${selectedClass
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 space-y-6 shadow-xl">
                          <div>
                            ...
                            <form onSubmit={handleAddStudent} className="grid grid-cols-6 gap-4">
                              <div className="col-span-6 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100/50">
                                <h4 className="text-sm font-bold text-emerald-800 mb-3 flex items-center gap-2"><UserPlus size={16}/> بيانات الطالب</h4>
                                <div className="grid grid-cols-6 gap-4">
                                  <input type="text" placeholder="الاسم الكامل" value={newStudent?.name || ''} onChange={(e) => setNewStudent({ ...newStudent, name: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all" />
                                  ...
                                  <select value={newStudent?.className || ''} onChange={(e) => setNewStudent({ ...newStudent, className: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all">
                              ...
                              <div className="col-span-6 bg-blue-50/50 p-4 rounded-xl border border-blue-100/50">
                                ...
                                <div className="grid grid-cols-6 gap-4">
                                  <input type="text" placeholder="اسم ولي الأمر" value={newStudent?.parentName || ''} onChange={(e) => setNewStudent({ ...newStudent, parentName: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all" />
                                  <input type="text" placeholder="رقم جوال ولي الأمر (اختياري)" value={newStudent?.parentPhone || ''} onChange={(e) => setNewStudent({ ...newStudent, parentPhone: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 font-mono focus:ring-2 focus:ring-blue-500/20 outline-none transition-all" />
                                  <input type="password" placeholder="كلمة مرور البوابة" value={newStudent?.parentPassword || ''} onChange={(e) => setNewStudent({ ...newStudent, parentPassword: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all" />
                                </div>
                              </div>
                              <div className="col-span-6 flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-3 shadow-sm">
                              ...
                              <button type="submit" className="btn-oversized col-span-6 bg-gradient-to-l from-emerald-600 to-emerald-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-emerald-500/30">
                                حفظ الطالب
                              </button>
                            </form>
                          </div>
                          <div className="border-t border-gray-200/50 pt-6">
                            <div className="flex flex-col gap-4 mb-6">
                              <div className="flex items-center justify-between flex-wrap gap-3">
                                ...
                                <div className="flex gap-2">
                                  <button type="button" onClick={handleExportStudents} className="btn-modern bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100 px-4 py-2 rounded-xl font-bold flex items-center gap-2 transition-all">
                                    <Download size={16}/> تصدير Excel
                                  </button>
                                  <label className="btn-modern bg-blue-50 text-blue-600 border border-blue-100 hover:bg-blue-100 px-4 py-2 rounded-xl font-bold flex items-center gap-2 cursor-pointer transition-all">
                              ...
                              <div className="flex items-center gap-3 bg-gray-50/50 p-3 rounded-2xl border border-gray-100">
                                <select value={classFilter} onChange={(e) => { setClassFilter(e.target.value); setStudentsPage(1); }} className="w-1/3 bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 text-gray-700 font-bold">
                        ...
                        <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 space-y-6 shadow-xl">
                          <div>
                            ...
                            <form onSubmit={handleAddStaff} className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <input type="password" placeholder="كلمة المرور" value={newStaff.password} onChange={(e) => setNewStaff({ ...newStaff, password: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all" />
                              </div>
                          ...
                          <div className="border-t border-gray-200/50 pt-6">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                              ...
                              <div className="flex items-center gap-2">
                                <input type="text" placeholder="ابحث بالاسم أو الحساب..." value={staffSearch} onChange={(e) => setStaffSearch(e.target.value)} className="bg-white border border-gray-200 rounded-xl py-2 px-4 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none w-48 md:w-64" />
                                <select value={staffFilter} onChange={(e) => setStaffFilter(e.target.value)} className="bg-white border border-gray-200 rounded-xl py-2 px-4 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                          ...
                          <div>
                            ...
                            <form onSubmit={(e) => { setNewStaff(prev => ({...prev, role: 'teacher'})); handleAddStaff(e); }} className="space-y-4">
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                ...
                                <select value={newStaff?.specialization || 'الرياضيات'} onChange={(e) => setNewStaff({ ...newStaff, specialization: e.target.value })} className="col-span-2 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                      ...
                      <div className="space-y-6">
                        <div className="border-b border-gray-200/50 pb-6 flex justify-between items-end">
                          ...
                          <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-emerald-100 flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-xs font-bold text-gray-500">إجمالي أرصدة الطلاب</p>
                              <p className="text-xl font-black text-emerald-600 tabular-nums">
                                {Object.values(wallets).reduce((a, b) => a + b, 0).toFixed(2)} <span className="text-sm">ر.س</span>
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-3xl p-8 shadow-xl shadow-emerald-900/20 text-white relative overflow-hidden flex flex-col">
                          ...
                          <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 space-y-4 shadow-xl">
                        ...
                        <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 shadow-xl mt-6">
                      ...
                      <div className="space-y-6">
                        <div className="border-b border-gray-200/50 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                          ...
                          <div className="flex bg-white rounded-xl p-1.5 shadow-sm border border-gray-100">
                            <button onClick={() => setComplaintFilter('all')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${complaintFilter === 'all' ? 'bg-primary text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>الكل</button>
                            <button onClick={() => setComplaintFilter('students')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${complaintFilter === 'students' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>الطلاب 🎓</button>
                            <button onClick={() => setComplaintFilter('parents')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${complaintFilter === 'parents' ? 'bg-indigo-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>أولياء الأمور 👨‍👩‍👦</button>
                          </div>
                        </div>

                        <div className="glass-card bg-white/70 rounded-3xl p-8 border border-white/50 shadow-xl">
                          {(() => {
                                  ...
                                  {filteredComplaints.slice().reverse().map((c, i) => {
                                    ...
                                    return (
                                      <motion.div
                                        ...
                                        key={c?.id || i}
                                        className={`p-6 bg-white/60 backdrop-blur-md border-2 rounded-2xl relative hover:shadow-md transition-all group ${isParent ? 'border-indigo-100' : 'border-orange-100'}`}
                                      >
                                        <button onClick={() => handleDeleteComplaint(c?.id)} className="absolute top-6 right-6 text-gray-400 hover:text-red-500 bg-white shadow-sm hover:shadow p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
                                          <X size={16} />
                                        </button>
                    ...
                    {activeTab === 'notifications' && (
                      <div className="space-y-6">
                        <div className="border-b border-gray-200/50 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                          ...
                          <div className="flex bg-white rounded-xl p-1.5 shadow-sm border border-gray-100">
                            <button onClick={() => { setNotifAudience('students'); setNotifTarget('all'); }} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${notifAudience === 'students' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>للطلاب 🎓</button>
                            <button onClick={() => { setNotifAudience('parents'); setNotifTarget('all'); }} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${notifAudience === 'parents' ? 'bg-indigo-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>لأولياء الأمور 👨‍👩‍👦</button>
                          </div>
                        </div>

                        <div className={`glass-card bg-white/70 rounded-3xl p-8 border shadow-xl space-y-5 transition-colors ${notifAudience === 'parents' ? 'border-indigo-100' : 'border-orange-100'}`}>
                          <div>
                            ...
                            <div className="relative">
                              <div className="flex gap-2 mb-3">
                                <select 
                                  ...
                                  className="w-1/3 border border-gray-200 rounded-2xl px-4 py-2.5 font-bold text-sm outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                                >
                                ...
                                <input
                                  ...
                                  className="flex-1 border border-gray-200 rounded-2xl px-4 py-2.5 font-bold text-sm outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                                />
                              </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            ...
                            <div key={i} className="bg-white/70 rounded-3xl p-6 border border-white/50 shadow-sm">
                                  ...
                                  setPasswordModalConfig({
                                    ...
                                    action: () => {
                                      if (item.special === 'wallets') {
                                        ...
                                        setWhitelist(whitelist.map(s => ({ ...s, totalClasses: 0, attendedClasses: 0, attendancePercentage: 0 })));
                                      } else {
                                        item.keys.forEach(k => localStorage.removeItem(k));
                                      }
                                      window.dispatchEvent(new Event('storage'));
                            ...
                            onClick={() => {
                              setPasswordModalConfig({
                                ...
                                action: () => {
                                  ...
                                  const resetWallets = {};
                                  whitelist.forEach(s => { resetWallets[s.id] = 0; });
                                  setWallets(resetWallets);
                                  setWhitelist(whitelist.map(s => ({ ...s, totalClasses: 0, attendedClasses: 0, attendancePercentage: 0 })));
                                  window.dispatchEvent(new Event('storage'));
                        ...
                        {/* 🎓 نظام ترحيل الطلاب */}
                        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-3xl p-6">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          <select value={graduatesYearFilter} onChange={(e) => setGraduatesYearFilter(e.target.value)}
                            className="p-3 rounded-xl border-2 border-gray-100 outline-none text-sm font-bold bg-white min-w-[160px]">
          ...
          {editingLesson && showScheduleOverlay && (
            ...
            >
              <motion.div
                ...
                className="glass-card bg-white/95 rounded-[2rem] p-8 w-full max-w-md space-y-5 shadow-2xl border border-white"
              >
          ...
          {editingStudent && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[500] bg-black/40 backdrop-blur-md flex items-center justify-center p-4">
              <motion.form
                ...
                animate={{ scale: 1, opacity: 1, y: 0 }}
                exit={{ scale: 0.9, opacity: 0, y: 20 }}
                className="glass-card bg-white/95 rounded-[2rem] p-8 w-full max-w-md space-y-4 shadow-2xl border border-white"
              >
                <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <input type="text" placeholder="الاسم الكامل" value={editingStudent.name || ''} onChange={(e) => setEditingStudent({ ...editingStudent, name: e.target.value })} className="col-span-2 w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-emerald-500/20" />
                    ...
                    <input type="password" placeholder="باسورد البوابة لولي الأمر" value={editingStudent.parentPassword || ''} onChange={(e) => setEditingStudent({ ...editingStudent, parentPassword: e.target.value })} className="w-full bg-blue-50/30 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-emerald-500/20" />

                    <select value={editingStudent.className || ''} onChange={(e) => setEditingStudent({ ...editingStudent, className: e.target.value })} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-emerald-500/20">
                      <option value="">اختر الفصل</option>
                      {(globalMaster.classes || []).map((cls, i) => <option key={i} value={cls}>{cls}</option>)}
                    </select>
                  </div>
        ...
        <AnimatePresence>
          {importErrors && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
              <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="bg-white rounded-3xl p-6 w-full max-w-2xl shadow-2xl border border-red-100 max-h-[90vh] flex flex-col">
                ...
                <div className="mt-6 pt-4 border-t border-gray-100 flex justify-end">
                  <button onClick={() => setImportErrors(null)} className="bg-gray-100 hover:bg-gray-200 text-gray-800 px-6 py-2.5 rounded-xl font-bold transition-all">إغلاق وتعديل الملف</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {walletConfirm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/40 backdrop-blur-md flex items-center justify-center p-4">
              <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="glass-card bg-white/95 rounded-[2rem] p-8 w-full max-w-md space-y-6 shadow-2xl border border-white text-center">
      ...
      {passwordModalConfig.isOpen && (
        <div className="fixed inset-0 z-[999999] bg-black/60 backdrop-blur-md flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) { setPasswordModalConfig({ ...passwordModalConfig, isOpen: false }); setPasswordInput(''); } }}>
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" dir="rtl">
            <div className={`p-6 border-b ${passwordModalConfig.isDanger ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'}`}>
              <div className="flex items-center gap-3">
        ...
        <div id="pdf-report-template" style={{ width: '210mm', minHeight: '297mm', padding: '20mm', backgroundColor: '#fff', color: '#000', direction: 'rtl', fontFamily: 'system-ui, sans-serif' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '2px solid #10b981', paddingBottom: '20px', marginBottom: '30px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <img src={logoImage} style={{ width: '80px', height: '80px', objectFit: 'contain', borderRadius: '10px' }} alt="Logo" />
              <div>
