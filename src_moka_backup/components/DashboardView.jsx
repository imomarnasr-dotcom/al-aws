...
const DashboardView = ({ studentData, searchQuery, onNavigateToSchedule }) => {
  ...
  const [behaviorLogsData] = useCloudStorage('behavior_logs', 'all_data', {});
  ...
  const nextClassData = useMemo(() => {
    ...
    const myExams = globalExams
      .filter(ex => {
        ...
        // إخفاء الاختبارات التي أتمها الطالب بالفعل
        if (ex.reports?.some(r => r.studentId === studentData?.personal?.id)) return false;
  ...
  }, [academicLessons, globalExams, studentData, currentTime]);
  ...
  const nextExam = useMemo(() => {
    ...
    const upcomingExams = globalExams
      ...
      .filter(ex => !ex.reports?.some(r => r.studentId === studentData?.personal?.id))
      .map(ex => {
         const exactDate = getExactExamDate(ex);
         let timeValue = 0;
         if (exactDate) {
            ...
            if (timeMatch) {
               ...
               // تمت إزالة منطق الإضافة التلقائية لـ 12 ساعة إذا لم يكن هناك modifier لمنع الخطأ
            }
  ...
  const behaviorLogs = useMemo(() => {
    return (behaviorLogsData || {})[studentData?.personal?.id] || [];
  }, [behaviorLogsData, studentData?.personal?.id]);
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6">
        ...
        {[
          ...
          { label: 'نقاط التميز', value: Math.max(0, dynamicPoints - ((spentPoints || {})[studentData?.personal?.id] || 0)).toLocaleString(), icon: <Star />, color: 'from-fuchsia-400 to-purple-600', subtext: `المكتسبة: ${dynamicPoints.toLocaleString()}` },
