    ...
    try {
      for (const item of collections) {
        ...
        if (Array.isArray(parsedData)) {
          // Save arrays as a single document under 'all_data' with a 'data' field
          await saveDoc(item.cloudCollection, "all_data", { data: parsedData });
        } else if (typeof parsedData === 'object' && parsedData !== null) {
          // If it's an object, save the whole object as a single document named "all_data"
          await saveDoc(item.cloudCollection, "all_data", { data: parsedData });
        }
      }
