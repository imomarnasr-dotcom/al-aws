...
const StudentProfile = ({ student, lastExamScore, onClose, onLogout, syncTrigger, onOpenStore }) => {
  ...
  const qrValue = JSON.stringify({
    ...
    balance: student?.cafeteria?.currentBalance
  });

  const [achievements, setAchievements] = useCloudStorage('achievements', 'all_data', {});
  const [exams] = useCloudStorage('exams', 'all_data', []);
  const [teacherTests] = useCloudStorage('tests', 'all_data', []);
  const [whitelist] = useCloudStorage('whitelist', 'all_data', []);
  ...
  useEffect(() => {
    ...
    const achievementsObj = achievements || {};
    let badges = achievementsObj[student.personal.id];
    ...
    if (changed) {
      achievementsObj[student.personal.id] = badges;
      ...
      setStudentNotifs(studentNotifs);
    }
  ...
  const [pinnedBadges, setPinnedBadges] = useState(() => {
    try {
      return pinnedBadges || ['وسام المواظب', 'وسام المتفوق', 'نجم الأوس', 'وسام القارئ'];
        ...
        >
          ...
          >
            ...
            <div className="bg-white rounded-3xl shadow-xl border-2 border-emerald-100 p-6">
              <QRCode value={qrValue} size={220} className="w-full h-full" />
            </div>
                ...
                <div className="bg-white/80 backdrop-blur-xl rounded-[28px] sm:rounded-[32px] p-5 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-center text-center sm:text-right">
                    <div className={`w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 bg-white rounded-full p-1.5 shrink-0 shadow-xl ring-4 ${purchasedFrameItem ? (purchasedFrameItem.id === 'frame_diamond' ? 'ring-cyan-300 shadow-cyan-200' : purchasedFrameItem.id === 'frame_fire' ? 'ring-red-500 shadow-red-300' : purchasedFrameItem.id === 'frame_neon' ? 'ring-fuchsia-500 shadow-fuchsia-300' : 'ring-yellow-400 shadow-yellow-200') : ringColor} transition-all duration-500 relative`}>
                      <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-full flex items-center justify-center text-4xl sm:text-5xl lg:text-6xl shadow-inner relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-5">
                  ...
                  >
                    ...
                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white rounded-2xl shadow-sm border border-gray-100 p-1.5 sm:p-2 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      <QRCode value={qrValue} size={80} className="w-full h-full" />
                    </div>
...
const App = () => {
  ...
  useEffect(() => {
    if (currentUser && !currentUser.role) {
      ...
      const currentWallets = { ...wallets }; if (currentWallets[id] === undefined) { currentWallets[id] = 0; setWallets(currentWallets); }
  ...
  const confirmPurchase = () => {
    ...
    setCafeteriaMenu(updatedMenu);

    // 3. Create Order
    const existingOrders = cafeteriaOrders || [];
    ...
    setCafeteriaOrders([newOrder, ...existingOrders].slice(0, 100));;
    ...
    const existingTransactions = walletTransactions || [];
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTeacherTests(updatedTeacher);
  ...
  // 🔥 إصلاح: تطبيق لون الواجهة المحفوظ للطالب فقط — المسئول والمعلم يرجعوا للأخضر
  useEffect(() => {
    try {
      if (currentUser && !currentUser.role) {
        // طالب — طبق اللون المحفوظ لو موجود
        const saved = JSON.parse(localStorage.getItem('moo_theme_color') || 'null');
        if (saved?.hex && saved?.rgb) {
          document.documentElement.style.setProperty('--primary-color', saved.hex);
          document.documentElement.style.setProperty('--primary-rgb', saved.rgb);
        }
      } else {
        // مسئول أو معلم أو غير مسجل — رجع للأخضر الافتراضي
        document.documentElement.style.setProperty('--primary-color', '#006C35');
        document.documentElement.style.setProperty('--primary-rgb', '0, 108, 53');
      }
    } catch { }
  }, [currentUser]);
  ...
  const handleMarkAllRead = useCallback(() => {
    ...
    try {
      const store = studentNotifs;
      const id = currentUser?.id;
      if (id && store[id]) {
        store[id] = store[id].map(n => ({ ...n, isNew: false }));
        setStudentNotifs(store);
      }
    } catch { /* تجاهل الخطأ */ }
    setSyncTrigger(prev => prev + 1);
  ...
  const student = useMemo(() => {
    ...
    try {
      ...
      const generalNotifs = globalNotifs;
                ...
                {!isLoading && (
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {currentPage === 'parent-dashboard' && <ParentDashboard studentData={currentUser} onLogout={() => setCurrentPage('landing')} />}
                      ...
                      {!hideStudentUI && (
                        <>
                          ...
                          {currentPage === 'settings' && <SettingsView studentData={student} setStudentData={handleSetStudentData} onLogout={() => setCurrentPage('landing')} />}
                          {currentPage === 'cafeteria' && <CafeteriaView studentData={student} cart={cart} setCart={setCart} searchQuery={searchQuery} studentBalance={studentBalance} onUpdateLocalBalance={(newBal) => {
                            const currentWallets = wallets || {};
                            if (currentWallets[student?.personal?.id] !== undefined) {
                              currentWallets[student.personal.id] = newBal;
                              setWallets(currentWallets);
