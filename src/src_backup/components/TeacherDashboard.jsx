...
const TeacherDashboard = ({ onLogout, currentTeacherUser }) => {
  ...
  const performSubmitAttendance = () => {
    ...
    // ًں”¥ النقطة 1+4: لا نخزّن أرقاماً مجمّعة بعد الآن — نخزّن السجل التفصيلي فقط
    // والنسبة تُحسب لحظياً عبر calculateStudentAttendance في كل الشاشات.
    // ًں”¥ النقطة 2: الطالب المعفي (isExempted) لا يُسجَّل له غياب إطلاقاً.
    const todayRecord = {};
  ...
  const submitAttendance = () => {
    if (!attendanceClass) return window.dispatchEvent(new CustomEvent('moo-toast', { detail: { message: '���� ������ �����', type: 'error' } }));
