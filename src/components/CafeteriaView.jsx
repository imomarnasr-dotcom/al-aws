...
const CafeteriaView = ({ studentData, cart, setCart, searchQuery = '', studentBalance }) => {
  ...
  return (
    <div className="flex-1 space-y-8 pb-10 overflow-x-hidden w-full max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:p-6">
        ...
        <div className="flex flex-wrap justify-center sm:justify-start gap-2 bg-white/60 backdrop-blur-xl p-1.5 rounded-2xl border border-white shadow-sm w-full md:w-auto">
          {[
            ...
            <button
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === tab.id
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:p-6">
        <div className="lg:col-span-4 relative group overflow-hidden bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-4 sm:p-8 shadow-xl shadow-emerald-500/20">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-4 sm:p-6">
          <div className="bg-white rounded-3xl p-4 sm:p-8 shadow-sm border border-slate-100 flex flex-col justify-center items-center text-center group hover:border-orange-200 transition-colors">
          ...
          <div className="bg-white rounded-3xl p-4 sm:p-8 shadow-sm border border-slate-100 flex flex-col justify-center items-center text-center group hover:border-blue-200 transition-colors">
      ...
      {activeTab === 'menu' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:p-6">
          {(filteredMenu || []).map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-3xl p-4 sm:p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full overflow-hidden relative"
            >
              {item.quantity === 0 && (
              ...
              <div className="relative mb-6 text-center bg-slate-50 rounded-2xl p-4 sm:p-6 group-hover:bg-orange-50 transition-colors">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="md:hidden flex flex-col divide-y divide-slate-100">
            {transactions.map((tx) => (
              <div key={tx.id} className="p-5 flex flex-col gap-4 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-slate-400">#{tx.id.toString().slice(-6)}</span>
                  <span className="font-black text-rose-500 text-sm bg-rose-50 px-3 py-1.5 rounded-lg">-{Number(tx.total).toFixed(2)} ر.س</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center text-lg shadow-sm border border-orange-100 shrink-0">🍽️</div>
                  <span className="font-bold text-slate-700 text-sm leading-snug">{tx.items || 'طلب مقصف'}</span>
                </div>
                <div className="flex flex-wrap gap-2 items-center justify-between mt-2">
                  <span className="text-[10px] text-slate-500 font-bold bg-slate-100 px-3 py-1.5 rounded-lg">
                    {formatLocalizedDate(tx.date)}
                  </span>
                  <div>
                    {tx.status === 'pending' ? (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-600 rounded-lg text-xs font-bold border border-slate-200">
                        <Clock size={14} /> قيد التجهيز
                      </span>
                    ) : tx.status === 'ready' ? (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 rounded-lg text-xs font-bold border border-orange-200">
      ...
      {showRechargeModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white max-w-sm w-full rounded-[32px] p-4 sm:p-8 shadow-2xl text-center relative border border-white animate-scale-in">
            <button 
              onClick={() => setShowRechargeModal(false)}
              className="absolute top-4 sm:p-6 left-6 text-slate-400 hover:text-rose-500 hover:bg-rose-50 p-2 rounded-full transition-colors"
            >
              <X size={20} />
            </button>
