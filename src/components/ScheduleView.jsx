...
import { getActiveWeekDays, getMasterTimeSlots, getGlobalMaster } from '../utils/dataManager';
...
const ScheduleView = ({ studentData, searchQuery: globalSearch }) => {
  ...
  const filteredSchedule = useMemo(() => {
    ...
    try {
      ...
      if (breakAfter < lessonsCount) {
        const masterSlots = getMasterTimeSlots(settings);
        const breakSlotTime = masterSlots[breakAfter];
        if (breakSlotTime && !lessons.find(l => l.isBreak || l.type === 'break')) {
          lessons.push({
            isBreak: true,
            type: 'break',
            subject: 'فسحة',
            time: breakSlotTime,
            day: activeDayData.name,
            classCode: studentClass
          });
        }
      }
    } catch (e) { console.error('Error injecting break:', e); }
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-10">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/50 p-5 sm:p-6 rounded-[32px] backdrop-blur-xl border border-white/60 shadow-sm">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
            <Calendar className="text-primary" size={28} />
            الجدول الدراسي
          </h1>
          <p className="text-gray-500 mt-1 font-medium text-sm sm:text-base">تابع حصصك ومواعيدك الأسبوعية</p>
        </div>
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className="w-full sm:w-72 bg-white/80 border border-gray-200/80 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 pr-11 pl-4 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium placeholder:text-gray-400 text-sm sm:text-base"
          />
        </div>
      </header>
        ...
        <div className="flex items-center gap-2 min-w-max">
          {SCHEDULE_DAYS.map((day) => (
            <button
              ...
              className={`px-4 sm:px-5 py-2 sm:py-3 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base transition-all ${activeDay === day.key
          ...
          {filteredSchedule.map((lesson, idx) => (
            ...
            >
              ...
              {(lesson.isBreak || lesson.type === 'break') ? (
                ...
                {/* 🎯 رأس الكارت: اللوجو + اسم المادة */}
                <div className="flex items-center gap-3 sm:gap-4 pb-4 border-b border-gray-50">
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br ${getSubjectColor(lesson.name || lesson.subject).replace('border-', 'shadow-')} text-white flex items-center justify-center text-2xl sm:text-3xl flex-shrink-0`}>
                    {getSubjectIcon(lesson.name || lesson.subject)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">المادة المقررة</p>
                    <h3 className="font-black text-gray-900 text-base sm:text-lg truncate">{lesson.name || lesson.subject || 'مادة غير محددة'}</h3>
                  </div>
                </div>

                <div className="flex items-center gap-3 text-gray-600 pt-1">
