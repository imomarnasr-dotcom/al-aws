...
const QRCard = ({ student }) => {
  const qrText = student && student.id ? JSON.stringify({ id: student.id, name: student.name }) : 'UNKNOWN';
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <p className="text-[10px] font-mono text-gray-400 mt-2 tracking-wider">{student?.id || 'UNKNOWN'}</p>
    </div>
  );
};
    ...
    <>
        ...
        >
          ...
          >
            ...
            <div className="bg-white rounded-3xl shadow-xl border-2 border-emerald-100 p-6">
              <QRCodeSVG value={JSON.stringify({ id: student.id, name: student.name })} size={220} level="H" includeMargin={false} fgColor="#111111" bgColor="#ffffff" className="w-full h-full" />
            </div>
            <p className="text-sm font-bold text-gray-500">{student?.name}</p>
            <p className="text-xs font-mono text-gray-400 bg-gray-50 px-4 py-2 rounded-xl">{student?.id}</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          <div key={i} className="glass-card rounded-3xl p-4 sm:p-6 shadow-sm flex flex-col sm:flex-row items-center sm:justify-between text-center sm:text-right gap-3 group hover:-translate-y-1 transition-all">
            <div>
              <p className="text-gray-400 text-xs font-bold mb-1">{s.label}</p>
              <p className={`text-xl sm:text-2xl font-black ${s.color}`}>{s.value}</p>
            </div>
            <div className={`w-10 h-10 sm:w-11 sm:h-11 ${s.bg} rounded-2xl flex items-center justify-center ${s.color}`}>{s.icon}</div>
          </div>
        ))}
      </div>
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <span className="font-black text-gray-700 text-lg text-center sm:text-right">نسبة الحضور الكلية</span>
          <span className={`text-sm font-black px-4 py-1.5 rounded-xl border text-center ${statusBg}`}>{statusLabel}</span>
        </div>
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
          <h2 className="text-lg sm:text-xl font-black text-gray-800 flex items-center gap-2 sm:gap-3">
            <QrCode size={20} className="text-blue-500" />
            بطاقة تأكيد الحضور
          </h2>
          <button
            onClick={() => setCardFlipped(f => !f)}
            className="w-full sm:w-auto flex justify-center items-center gap-2 text-xs font-bold text-gray-500 hover:text-blue-600 transition-colors bg-gray-50 px-4 py-2.5 sm:py-2 rounded-xl border border-gray-200"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-center">
          ...
          {!cardFlipped ? (
            ...
            <div className="flex flex-col items-center gap-4 flex-shrink-0">
              <div 
                className="bg-white rounded-3xl p-4 shadow-lg border border-gray-100 cursor-pointer hover:shadow-xl hover:scale-105 transition-all"
                onClick={() => setShowQR(true)}
              >
                <QRCard student={student} />
              </div>
              <p className="text-xs text-gray-400 font-bold text-center cursor-pointer hover:text-blue-500 transition-colors" onClick={() => setShowQR(true)}>
                اضغط لعرض الكود بالكامل
              </p>
            </div>
          )}
      ...
      <div className="glass-card rounded-3xl shadow-sm overflow-hidden border border-white/20">
        <div className="p-6 sm:p-8 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-[10px] sm:text-xs font-black text-gray-400 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">{absentCount} يوم</span>
        </div>
