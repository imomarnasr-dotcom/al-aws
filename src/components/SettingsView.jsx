...
const SettingsView = ({ studentData, setStudentData, onLogout, studentPassword }) => {
  ...
  const handleConfirmLogout = () => {
    if (!logoutPassword) {
      setLogoutError('الرجاء إدخال كلمة المرور');
      return;
    }
    
    let actualPassword = studentPassword; // Default to prop if passed
    try {
      const wl = JSON.parse(localStorage.getItem('moo_whitelist') || '[]');
      const me = wl.find(s => s.id === studentData?.personal?.id);
      if (me && me.password) {
        actualPassword = me.password;
      }
    } catch { /* ignore */ }

    if (logoutPassword !== actualPassword) {
  ...
  const handleChangePassword = () => {
    let realPassword = studentPassword || studentData?.password || studentData?.personal?.password || '';
    try {
      const wl = JSON.parse(localStorage.getItem('moo_whitelist') || '[]');
      const me = wl.find(s => s.id === studentData?.personal?.id);
      if (me && me.password) {
        realPassword = me.password;
      }
    } catch { /* ignore */ }
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl transition-all shadow-md flex justify-center items-center gap-2 mt-4 mb-6"
                  >
                    تأكيد وحفظ الثيمات
                  </button>
                  ...
                  <div className="p-6 bg-gray-50 rounded-3xl border border-gray-100 flex items-center justify-between">
                    <div>
                      <label className="text-sm font-black text-gray-800 block mb-1">المؤشر السحري (Magic Cursor)</label>
                      <p className="text-xs font-bold text-gray-500">تفعيل أو تعطيل تأثير المؤشر التفاعلي الذي يتبع الماوس.</p>
                    </div>
                    <button
                      onClick={toggleMagicCursor}
                      className={`relative w-14 h-8 rounded-full transition-colors duration-300 ${magicCursor ? 'bg-primary' : 'bg-gray-300'}`}
                    >
                      <div className={`absolute top-1 w-6 h-6 rounded-full bg-white transition-all duration-300 shadow-sm ${magicCursor ? 'left-1' : 'left-7'}`}></div>
                    </button>
                  </div>
